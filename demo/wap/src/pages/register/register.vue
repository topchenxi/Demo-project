  ...
  <div class="register-out-wrap">
    <header class="border-1px">
      ...
      <router-link to="/login"
                   replace
                   slot="next"
                   class="c-head-next" :style="{color:'#FF0030'}">Sign in</router-link>
    </header>
    <ul class="register-wrap margin-b">
      <router-link to="/selectCountry"
                   tag="li"
                   class="country-region-wrap"><span class="country-region">Country / Region</span><span class="country-name"
              v-if="countryInfo">{{countryInfo.countryEnName}}</span></router-link>
      <li class="border-1px">
        <input type="text"
               placeholder="Email"
               v-model.trim="email">
    ...
    <div @click="checkInput"
         class="signin-btn"
         :style="{background: btnBg}">Sign Up</div>
  </div>
</template>
