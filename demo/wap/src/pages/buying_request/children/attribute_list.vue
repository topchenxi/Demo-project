...
<template>
  <div class="attributes-wrap">
    ...
    <ul class="attributs-ul">
      <router-link class="border-1px" :to="'/attributeDetail/'+ item.propertyEnName + '/' + JSON.stringify(item.propertyValues) + '/' + index" tag="li" v-for="(item, index) of properties" :key="index"> {{item.propertyEnName}}
        <span class="get-property">{{propertiesValue[index]}}</span>
      </router-link>
    </ul>
    <div class="submit" @click="ifLogin">
      <span>Submit</span>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      sendRequest() {
        ...
        this.axios({
            ...
            params: CFEC.addConfig(requsetParams)
          })
          .then((res) => {
            if (res.data.status === "success") {
              let categoryId = requsetParams.procurementStr.productLastCategoryId;
              let source=CFEC.getUrlParam('source');
              let gaRequestParams = `Wap_Others_${source}_categoryId=${categoryId}`;
              ga('send', 'event', 'BuyerAction', 'BuyingRequest', gaRequestParams);
