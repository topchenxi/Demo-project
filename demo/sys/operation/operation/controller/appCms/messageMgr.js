define(['../module'], function(ng) {
    ...
    ng.controller('addMsgPushCtrl', [
        ...
        function($scope, $rootScope, $filter, messageMgrService, commonTool, $location, ngDialog) {
            ...
            let method = {
                ...
                isUnique: function(checkItem) {
                    if (!vm.selectTagArray.length) return true;

                    for (let i = 0, len = vm.selectTagArray.length; i < len; i++) {
                        let item = vm.selectTagArray[i];
                ...
                initJson: function() {
                    ...
                    if (!vm.selectTagArray.length) return '';

                    let tagArray = [];
                    for (let i = 0, len = vm.selectTagArray.length; i < len; i++) {
                        let item = vm.selectTagArray[i];
                        if (vm.tagIsNot == 1) {
                            tagArray.push({ 'tag': item.tagName });
                        } else {
                            tagArray.push({ 'not': { 'tag': item.tagName } });
                        }
                    }
                    let obj = {
                        'where': {
            ...
            tools = angular.extend(tools, {
                ...
                addToSelect: function(item) {
                    if (!method.isUnique(item)) return;
                    let tmpItem = angular.extend({}, item);
                    vm.selectTagArray.push(tmpItem);
                },
                delFromSelect: function(index) {
                    vm.selectTagArray.splice(index, 1);
                }
            });
    ...
    ng.controller('userTagMgrCtrl', [
        ...
        function($scope, $rootScope, $location, commonService, messageMgrService, commonTool, ngDialog) {
            ...
            class UserTag extends List {
                ...
                getList() {
                    let self = this;
                    messageMgrService.getUserTagList(commonTool.filterParam(this.paging)).success(function(data) {
                        if (data.status === 'success') {
                            self.list = data.page.items;
                            self.total = data.page.total;
                        }
    ...
    ng.controller('addUserTagCtrl', [
        ...
        'Upload',
        'api',
        function($scope, $rootScope, $location, commonService, messageMgrService, commonTool, ngDialog, Upload, api) {
            ...
            tools = angular.extend(tools, {
                submit: function() {
                    ...
                    if (commonTool.isEmpty(tagInfo.userIds) && commonTool.isEmpty(vm.attachFile)) {
                        $rootScope.alertMsgService.setMessage("请先填写userIds", 'warning');
                        return;
                    }
                ...
                uploadFile: function(files) {
                    ...
                    if ((!/.*\.(txt)$/.test(myFile.name)) || myFile.size > 2 * 1024 * 1024) {
                        $rootScope.alertMsgService.setMessage("请重新支持格式：txt;文件大小不超过2M", 'warning');
                        return false;
                    }
                    Upload.upload({
                        ...
                        vm.attachFile = new AttachFile(config.file.name, data.data.abslouteUrl + data.data.url);
                    });
                },
                delFile: function() {
                    vm.attachFile = null;
