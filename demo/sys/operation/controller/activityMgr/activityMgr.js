define(['../module'], function(ng) {
    ...
    ng.factory('activityMgrService', ['$http', 'api', function($http, api) {
        ...
        const getActivitiesList = params => $http.get(api.getActivitiesList, { 'params': params });
        const getActivitiesDetail = params => $http.get(api.getActivitiesDetail, { 'params': params });
        const getMemPackageList = params => $http.get(api.getMemPackageList, { 'params': params });
        const getEnrollList = params => $http.get(api.getEnrollList, { 'params': params });
        const getOnlineEnrollList = params => $http.get(api.getOnlineEnrollList, { 'params': params });
        const getOfflineEnrollList = params => $http.get(api.getOfflineEnrollList, { 'params': params });
        const getOfflineSellerDetail = params => $http.get(api.getOfflineSellerDetail, { 'params': params });
        const auditActivity = params => $http.post(api.auditActivity, params);
        const saveActivityInfo = params => $http.post(api.saveActivityInfo, params);
        const auditApplyOnline = params => $http.post(api.auditApplyOnline, params);
        const auditApplyOffline = params => $http.post(api.auditApplyOffline, params);
        return {
            getActivitiesList,
            getActivitiesDetail,
            auditActivity,
            ...
            getEnrollList,
            getOnlineEnrollList,
            getOfflineEnrollList,
            getOfflineSellerDetail,
            auditApplyOnline,
            auditApplyOffline
        };
    }])
    ...
    ng.controller('activityListCtrl', [
        ...
        function(
            ...
            class ActivityList extends List {
                ...
                getList() {
                    activityMgrService.getActivitiesList(commonTool.filterParam(this.paging)).success(data => {
                ...
                reset() {
                    super.reset();
                    this.paging.status = this.currentStatus;
                }
                addActivity() {
            ...
            let vm = $scope.vm = new ActivityList(defaultPaging);
        }
    ])
    ...
    ng.controller('enrollListCtrl', [
        ...
        function(
            ...
            class EnrollList extends List {
                constructor(defaultPaging) {
                    super(defaultPaging);
                }
                getList() {
                    let self = this;
                    activityMgrService.getEnrollList(commonTool.filterParam(this.paging)).success(function(data) {
                        if (data.status === 'success') {
            ...
            let vm = $scope.vm = new EnrollList(defaultPaging);
            ...
            tools = angular.extend(tools, {
                ...
                setInfo: function(detailInfo) {
                    ...
                    if (detailInfo.uploadTemp) {
                        ...
                        activityInfo.uploadTempName = '附件';
                    }
                ...
                saveInfo: function() {
                    ...
                    let textHtml = $('.textarea-cfec').html();

                    if (commonTool.isEmpty(textHtml)) {
                    ...
                    let params = Object.assign({}, activityInfo);

                    params.eventDesc = textHtml;

                    activityMgrService.saveActivityInfo(params).success(function(data) {
                        if (data.status === 'success') {
                            $rootScope.alertMsgService.setMessage("保存成功", 'success');
                            $location.url('/activityMgr/activityList');
                        } else {
                ...
                uploadImg: function(files) {
                    if (files && files.length > 0) {
                        var file = files[0];
                        if ((!/.*\.(png)|(PNG)|(jpg)|(JPG)|(gif)|(GIF)|(psd)|(PSD)$/.test(file.name))) {
    ...
    ng.controller('onLineListCtrl', [
        ...
        function(
            ...
            tools = angular.extend(tools, {
                ...
                isVedio: function(str) {
                    if (!str) return false;
                    var postfix = '';
                    index = str.lastIndexOf('.');
                    if (index != -1) postfix = str.substring(index + 1, str.length);
    ...
    ng.filter('materialLinkFiler', function() {
        return function(materialId, materialType) {
            return materialType == 1 ? `${ecDomain}/products/product-${materialId}.html` : `${ecDomain}/china-supplier/${materialId}.html`;
        };
    });
