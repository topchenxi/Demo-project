...
if (lang != 'zh'){
    laydateLang = {
        ...
        sat: 'Sat',
        sun: 'Sun',
        errorTip1: 'Date is not in conformity with the format.',
        errorTip2: 'Date is not in the period of validity.'
    ...
    laydateLang = {
        ...
        sat: '六',
        sun: '日',
        errorTip1: '日期不符合格式，请重新选择。',
        errorTip2: '日期不在有效期内，请重新选择。'
    };
}

    define('calendar', ['css!calendarCss'], function(){
    //全局配置，如果采用默认均不需要改动
    var config =  {
        path: '', //laydate所在路径
    ...
    var as = ['laydate_box', 'laydate_void', 'laydate_click', 'LayDateSkin', 'skins/', '/laydate.css' ,'laydate_markTheme','laydate_mark'];
    ...
    Dates.run = function(options){
        ...
        // 2016-07-21 by ken 增加日期禁用标记样式
        if(options.enabledMark != '[]' && options.enabledMark != undefined){
            if(!Dates.hasClass(S('#laydate_box'),as[6])){
                Dates.addClass(S('#laydate_box'),as[6]);
            }
        }else{
            if(Dates.hasClass(S('#laydate_box'),as[6])){
                Dates.removeClass(S('#laydate_box'),as[6]);
            }
        }
    };
    ...
    Dates.check = function(){
        ...
        if(value.replace(/\s/g, '') !== ''){
            if(!exp.test(value)){
                Dates.elem[as.elemv] = '';
                Dates.msg(laydateLang.errorTip1);
                return 1;
            } else if(isvoid[0]){
                Dates.elem[as.elemv] = '';
                Dates.msg(laydateLang.errorTip2);
                return 1;
            } else {
    ...
    Dates.viewDate = function(Y, M, D){
        ...
        Dates.each(as.tds, function(i, elem){
            ...
            // enabledMark by ken 2016-07-21
            // console.log(Dates.options.enabledMark)
            if(Dates.options.enabledMark != undefined){
                var span = doc[creat]('span');
                ...
                var enabledMarkJson = JSON.parse(Dates.options.enabledMark);
                
                if(enabledMarkJson.length > 0){
                    
                    // 引用
                    var ed = enabledMarkJson;
                    // 禁用列表
                    var enabledList = [];
                    // 一天毫秒数
                    var e_ms = 24*60*60*1000;
                    // 生成禁用列表
                    for(var k = 0;k<ed.length;k++){
                        var e_start = ed[k].start;
                        var e_end = ed[k].end;
                        var e_enabledText = ed[k].text || '';
                        // 转时间戳
                        var e_start_date = new Date(e_start).getTime();
                        var e_end_date = new Date(e_end).getTime();
                        // 计算日期区间之间的天数
                        var dateRange = (e_end_date - e_start_date)/e_ms;
                        // 把日期区间第一天加进禁用列表
                        // enabledList.push(e_start);
                        // 遍历区间生成日期加入禁用列表
                        if(dateRange > 0){
                            for(var p=0;p<=dateRange;p++){
                                var e_ms_temp = e_start_date + e_ms*p;
                                if(e_ms_temp <= e_end_date){
                                    var e_ms_temp_json = {};
                                    var e_ms_temp_date = new Date(e_ms_temp);
                                    e_ms_temp_json.date = Dates.parse(
                                        [e_ms_temp_date.getFullYear(), e_ms_temp_date.getMonth()+1, e_ms_temp_date.getDate()],
                                        [e_ms_temp_date.getHours(), e_ms_temp_date.getMinutes(), e_ms_temp_date.getSeconds()],
                                        'YYYY-MM-DD'
                                    );
                                    e_ms_temp_json.text = e_enabledText;

                                    enabledList.push(e_ms_temp_json);
                                }
                            }
                        }else{
                            // 日期区间start和end为同一天
                            var e_ms_temp_json = {};
                            e_ms_temp_json.date = e_start;
                            e_ms_temp_json.text = e_enabledText;
                            enabledList.push(e_ms_temp_json);
                        }
                    }
                    ...
                    // 对比禁用列表输出到界面
                    for(var enabledTemp = 0;enabledTemp<enabledList.length;enabledTemp++){
                        var edArr_temp = enabledList[enabledTemp];
                        var ed_YY = YY;
                        var ed_MM = String(MM).length<2?'0'+MM:MM;
                        var ed_DD = String(DD).length<2?'0'+DD:DD;
                        var current_dom_date = ed_YY+'-'+ed_MM+'-'+ed_DD;
                        // console.log(
                        //     edArr_temp,
                        //     current_dom_date,
                        //     edArr_temp==current_dom_date
                        // );
                        if(edArr_temp.date == current_dom_date){
                            Dates.addClass(elem, as[7]);
                            Dates.addClass(elem, as[1]);
                            span.innerHTML = edArr_temp.text;
                        }
                    }
                }
    ...
    Dates.viewtb = (function(){
        var tr, view = [], weeks = [laydateLang.sun, laydateLang.mon, laydateLang.tue, laydateLang.wed ,laydateLang.thur, laydateLang.fri,  laydateLang.sat];
    ...
    Dates.view = function(elem, options){
        ...
        if(!Dates.box){
            ...
            div.id = as[0];
            div.className = as[0];
            // 根据传参字段判断是否属于日期标记类型，如果是增加日期标记class
            if(Dates.options.enabledMark != undefined && Dates.options.enabledMark != '[]'){
                div.className = as[0] + ' ' + as[6];
            }
            ...
            +'<div class="laydate_bottom">'
              +'<ul id="laydate_hms">'
                +'<li class="laydate_sj">' + laydateLang.time + '</li>'
              ...
              +'<div class="laydate_btn">'
                +'<a id="laydate_clear">' + laydateLang.empty + '</a>'
                +'<a id="laydate_today">' + laydateLang.today + '</a>'
                +'<a id="laydate_ok">' + laydateLang.confirm + '</a>'
              +'</div>'
    ...
    Dates.events = function(){
        ...
        Dates.on(as.oclear, 'click', function(){
            typeof Dates.options.clear === 'function' && Dates.options.clear();
            Dates.elem[as.elemv] = '';
            Dates.close();
        });
        ...
        log.hmss = [laydateLang.hour, laydateLang.minute, laydateLang.second];
        log.hmsarr = [];
        //生成时分秒或警告信息
        Dates.msg = function(i, title){
            var str = '<div class="laydte_hsmtex">'+ (title || laydateLang.title) +'<span>×</span></div>';
            if(typeof i === 'string'){
                str += '<p>'+ i +'</p>';
                Dates.shde(S('#'+as[0]));
