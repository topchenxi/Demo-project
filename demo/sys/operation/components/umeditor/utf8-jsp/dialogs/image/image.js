(function() {
    ...
    var utils = UM.utils,
        ...
        Base = {
            ...
            callback: function(editor, $w, url, state) {
                // update by cxl 2015-6-19
                if (state == "SUCCESS") {
                    //显示图片计数+1
                    Upload.showCount++;
                    //   console.log('path==',editor.options.imagePath + url);
    ...
    var Upload = {
        ...
        uploadComplete: function(r) {
            ...
            try {
                // update by cxl 2015-6-19
                r = r.substring(r.indexOf('{'), r.lastIndexOf('}'));
                r = r.replace(/"/g, '');
                //var json = JSON.stringify(r);//$.parseJSON(r); //eval('('+r+')');

                var url = r.replace(/.*abslouteUrl:([\w\W]*?),[\w\W]*/, "$1") + r.replace(/.*,url:([\w\W]*?)}[\w\W]*/, "$1");
                var status = 'SUCCESS';
                Base.callback(me.editor, me.dialog, url, status);
            } catch (e) {
        ...
        submit: function(callback) {
            ...
            $(me.dialog).delegate(".edui-image-file", "change", function(e) {
                ...
                var tt = me.dialog.baseURI;

                var fileSize = $(this)[0].files[0].size;

                if (fileSize > 2 * 1024 * 1024) {
                    alert('图片太大,不能超过2M,请重新选择');
                    Upload.updateInput(input);
                    return;
                }


                $('<iframe name="up" id="theup" style="display: none"></iframe>').insertBefore(me.dialog).on('load', function() {
                    //alert("joooo");
                    //alert($(document.getElementById('theup').contentWindow.document.body).html());
