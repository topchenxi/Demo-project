    ...
    ng.factory('exhibitionService', ['$http', 'api',
            function($http, api) {
                return {
                    ...
                    importExhibiton: function(params) {
                        return $http.get(api.importExhibiton, {
                            'params': params
                        });
                    }
            ...
            function($scope, $rootScope, ngDialog,
                ...
                var method = {
                    ...
                    getSearchTypeArray: function() {
                        return [
                            new this.myObject('公司中文名称', 'companyName'),
                            new this.myObject('公司英文名称', 'companyEName'),
                            new this.myObject('sellerId', 'sellerId'),
                            new this.myObject('ibeaconId', 'ibeaconId')
                        ];
                    }
                };
        ...
        .controller('sellerBindExhibitionCtrl', ['$scope', '$rootScope', 'ngDialog',
            ...
            function($scope, $rootScope, ngDialog,
                ...
                tools = $.extend(tools, {
                    ...
                    exibitorChx: function() {
                        let obj = {
                            '10': 1000,
                            '20': 2000,
                            '30': 5000
                        }
                        paging.sendTotal = obj[paging.level];
                    },
                    saveBindExhibition: function() {
                        ...
                        if (commonTool.isEmpty(paging.sendTotal)) {
                            $rootScope.alertMsgService.setMessage('请选择条数', 'warning');
                            return;
                        }
                    ...
                    getExhibitionDetail: function(promotionId) {
                        exhibitionService.getExhibitionDetail(commonTool.filterParam({ promotionId: promotionId })).success(function(data) {
                            ...
                            paging.sendTotal = data.data.sendTotal;
                            paging.remark = data.data.remark;
                        });
                    }
                });
            ...
            function($scope, $rootScope, ngDialog,
                ...
                tools = $.extend(tools, {
                    ...
                    uploadFile: function(file) {
                        ...
                        var myFile = file[0];
                        if ((!/.*\.(xls)|(xlsx)|(XLS)|(XLSX)$/.test(myFile.name)) || myFile.size > 2 * 1024 * 1024) {
                            $rootScope.alertMsgService.setMessage("请重新支持格式：xls/xlsx;文件大小不超过2M", 'warning');
                            return false;
                        }
                        commonService.upload(myFile).success(function(data, status, headers, config) {
                            var obj = {
                                filePath: data.data.url
                            };
                            exhibitionService.importExhibiton(obj).success(function(data) {
