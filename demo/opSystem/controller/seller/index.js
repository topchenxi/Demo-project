define(['../module'], function(ng) {
    ng.factory('sellerServer', ['$http', 'api',
        function($http, api) {
            return {
                ...
                auditIndex: function(params) {
                    return $http.get(api.auditIndex, {
                        params: params
                    });
                },
                ...
                sellerShopAudit: function(params) {
                    return $http.post(api.sellerShopAudit, params);
                },
                ...
                // 批量设置免审
                setBatchTrial: function(params) {
                    return $http.post(api.batchTrial, params);
                },
    ...
    ng.controller('SellerCtrl', [
        ...
        function(
            ...
            var paging = $scope.paging = {
                ...
                statusTab: $location.search().type ? $location.search().type : 2,
            ...
            tools = $.extend(tools, {
                ...
                auditIndex: function() {
                    sellerServer.auditIndex().success(function(data) {
                        if (data.status === 'success') {
                            $rootScope.alertMsgService.setMessage("已操作完成，请在待审核列表中查看数据", 'success');
                            tools.getSellerLst();
                        }
                    })
                },
                ...
                saveInfo: function(type) {
                    ...
                    ngDialog.open({
                        ...
                        animation: true,
                        width: 500,
                        templateUrl: 'setquoteDlg.html',
                        controller: ['$scope', function($scope) {
                            var type = vm.setquote == '批量设置报价免审' ? '确认设置报价免审吗？' : '确认取消报价免审吗？'
                            $scope.message = type;
                        }]
                    });
                },
                submits: function() {
                    var type = vm.setquote == "批量设置报价免审" ? 1 : 0
                    var options = {
                        isFreeAduit: type,
                        sellerIds: vm.selectionItme.join(",")
                    }
                    sellerServer.setBatchTrial(options).success(function(data) {
        ...
        function(
            ...
            **************************************************/
                ...
                getdetail: function(sellerId) {
                    sellerServer.getSellerDetail(tools.sellerId).success(function(data) {
                        if ('success' == data.status) {
                            ...
                            vm.item.companyInfo.companyEnName = vm.item.companyInfo.companyEnName.replace(/\s/g, '&nbsp');
                ...
                modifyData: function(type, val) {
                    ...
                    switch (type) {
                        ...
                        case 'mainCategoryId':
                            ...
                            break;  
                        case 'leaderName':
                            vm.modifyData.typeName = '企业负责人姓名';
                            vm.modifyData.typeNo = 5;
                            break;
                        case 'leaderEmail':
                            vm.modifyData.typeName = '企业负责人邮箱';
                            vm.modifyData.typeNo =6;
                            break;
                        case 'leaderMobile':
                            vm.modifyData.typeName = '企业负责人手机';
                            vm.modifyData.typeNo = 7;
                            break;
                        case 'leaderWechat':
                            vm.modifyData.typeName = '企业负责人微信';
                            vm.modifyData.typeNo = 8;
                            break;
                        case 'leaderWhatsapp':
                            vm.modifyData.typeName = '企业负责人Whatsapp';
                            vm.modifyData.typeNo = 9;
                            break;
                    }
                    ...
                    vm.modifyDataDlg = ngDialog.open({
                        ...
                        controller: ['$scope', function($scope) {
                            ...
                            $scope.submit = function() {
                                ...
                                // 校验
                                if (vm.modifyData.type == 'email'||vm.modifyData.type =='leaderEmail') {
                                    if (!/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((.[a-zA-Z0-9_-]{2,3}){1,2})$/.test(params.sellerInfoValue)) {
                                        $rootScope.alertMsgService.setMessage('请输入正确的' + vm.modifyData.typeName, 'warning');
                                        return;
                                    }
                                }
                                if (vm.modifyData.type == 'mobile'||vm.modifyData.type == 'leaderMobile') {
                                    if (!/0?(13|14|15|17|18)[0-9]{9}/.test(params.sellerInfoValue)) {
                                        $rootScope.alertMsgService.setMessage('请输入正确的' + vm.modifyData.typeName, 'warning');
                                        return;
                                    }
                                }
                                if (vm.modifyData.type == 'leaderName') {
                                    if (/[\w]{11,}/.test(params.sellerInfoValue)) {
                                        $rootScope.alertMsgService.setMessage('请输入少于10个字符' + vm.modifyData.typeName, 'warning');
                                        return;
                                    }
                                }
                                 if (vm.modifyData.type == 'leaderWechat') {
                                    if (!/^[a-zA-Z0-9\_\-]{5,}$/.test(params.sellerInfoValue)) {
                                        $rootScope.alertMsgService.setMessage('请输入5位以上只能包含数字、英文、下划线，中横线' + vm.modifyData.typeName, 'warning');
                                        return;
                                    }
                                }
                                if (vm.modifyData.type == 'leaderWhatsapp') {
                                    if (/^[\w]{21,}$/.test(params.sellerInfoValue)) {
                                        $rootScope.alertMsgService.setMessage('请输入不超过20字符' + vm.modifyData.typeName, 'warning');
                                        return;
                                    }
                                }
            ...
            sellerShopTools = $.extend(sellerShopTools, {
                ...
                chxShopInfoAudit: function(type) {
                    ngDialog.open({
                        ...
                        controller: ['$scope', function($scope) {
                            var vm = $scope.vm = {
                                ...
                                submit: function() {
                                    ...
                                    sellerServer.sellerShopAudit(params).success(function(data) {
                                        if (data.status === 'success') {
                                            sellerShopTools.getSellerShopInfo();
    ...
    ng.filter("shopStatusFilter", function() {
        return function(auditStatus) {
            switch (auditStatus) {
                case 2:
                    return '待审核';
    ...
    ng.filter("shopOnlineStatusFilter", function() {
        return function(auditStatus) {
            switch (auditStatus) {
                case 0:
                    return '未开通';
