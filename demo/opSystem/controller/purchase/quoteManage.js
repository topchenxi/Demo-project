...
define(['../module'], function(ng) {
    ng.factory('quoteService', ['$http', 'api',
        function($http, api) {
            return {
                ...
                //设置报价免审时段
                setFreeExamine: function(params){
                	return $http.post(api.freeExamine, params);
                },
                // 批量设置免审
                setBatchTrial: function(params){
                	return $http.post(api.batchTrial, params);
                },
                getreviewRule :function(params){
                	return $http.post(api.reviewRule, params);
                }
            };
        }
    ])
    ...
    //报价免审设置
    ng.controller('exemptReviewCtrl', ['$scope', '$rootScope', 'ngDialog', '$controller', '$location', 'quoteService', 'commonService', 'commonTool',
            function($scope, $rootScope, ngDialog, $controller, $location, quoteService, commonService, commonToo) {
            	var vm=$scope.vm={};
            	var tool=$scope.tool={};            	
            	    vm.item={
            	    	day:"0",
            	    	week:[{val:"1",dates:"周一",num:null},{val:"2",dates:"周二",num:null},{val:"3",dates:"周三",num:null},{val:"4",dates:"周四",num:null},{val:"5",dates:"周五",num:null},{val:"6",dates:"周六",num:null},{val:"7",dates:"周日",num:null}],
            	    	fromData:"",
            	    	toData:"",
            	    	minData:""
            	    };         	   
                ...
                tool=$.extend(tool,{
                	submit:function(){                		
	                		...
	                		if(type=="1"){	                			
		                		for(var n=0;n<vm.item.week.length;n++){		                			
		                			if(vm.item.week[n].num){
		                				rule.push(vm.item.week[n].num)
		                			}		                		    	
		                		}	
		                		 if(rule.length>0){
		                		 	data.type=type;
		                		 	...
		                		 	this.saveInfo(data)
		                		 	 ...
		                		 	 $rootScope.alertMsgService.setMessage("请选择天数", 'warning');
		                		 }		             		  
	                		}
	                		if(type=="2"){
                                var starTime=this.getTime(vm.item.fromData);
                                var endTime=this.getTime(vm.item.toData);
                                var ruleTime=new Date().getTime()
                                                                
	                		    if(vm.item.fromData==""||vm.item.toData==""){
	                		       	 $rootScope.alertMsgService.setMessage("输入时间段不能为空", 'warning'); 
	                		       	 return
	                		    }
	                		    
	                		    if(starTime<ruleTime){
                                	$rootScope.alertMsgService.setMessage("开始时间不小于当前时间", 'warning');
                                	return 
                                }
                                if(starTime>=endTime){
                                	$rootScope.alertMsgService.setMessage("开始时间不能大于等于结束时间", 'warning');
                                	return 
                                }
	                		       rule.push(vm.item.fromData);
	                		       rule.push(vm.item.toData);
	                		        data.type=type
	                		        data.rule=rule.join(",");	                		        
		                		   this.saveInfo(data);
	                		}
                      ...
                      saveInfo: function(params){
                      	 quoteService.setFreeExamine(params).success(function(data) {
				              if (data.status === 'success') {
                ...
                init_data()
                function init_data() {
                     quoteService.getreviewRule().success(function(data){
                     	  var params=data.data;
                     	  vm.item.day=params.type;
                     	 
                    	  if(params.type==1&&params.rule){
                    	  	
                    	 params.rule= params.rule.split(",")
                     	  	 for(var n=0;n<vm.item.week.length;n++){                     	  	
	                     	  	 for(var j=0;j<params.rule.length;j++){
	                     	  	 	if(vm.item.week[n].val==params.rule[j]){	                     	  	 		
			                			vm.item.week[n].num=params.rule[j]-0;			                			
			                			params.rule.splice(j,1)
			                	    }	                     	  	 	
	                     	  	 }           	   		                		    	
		                     }
                   	      }
                    	  if(params.type==2&&params.rule){
                    	  	   params.rule= params.rule.split(",")
                     	  	   vm.item.fromData=params.rule[0]
                     	  	   vm.item.toData=params.rule[1]
	                      }
                     })
                };
         }
