        ...
        .controller('LoginCtrl', ['$scope', '$rootScope', '$location', '$uibModal', 'loginService', 'commonService', 'commonTool',
            function($scope, $rootScope, $location, $uibModal, loginService, commonService, commonTool) {
                ...
                tools = $.extend(tools, {
                    ...
                    checkLogin: function() {
                        ...
                        loginService.login(params).error(function(data) {
                            ...
                            loginService.account().success(function(data) {
                                ...
                                if (tools.isOtherForm) {
                                    window.location.href = coommerurl + '/#' + search.returnUrl;
                                } else {
                                    $location.path('/index');
                                }
                            })
                    ...
                    isOtherForm: function() {
                        if (!commonTool.isEmpty(search.username) && !commonTool.isEmpty(search.password) && !commonTool.isEmpty(search.returnUrl)) {
                            return true;
                        } else {
                            return false;
                        }
                ...
                if (tools.isOtherForm) {
                    vm.username = search.username;
                    vm.password = search.password;
                    tools.checkLogin();
                }
            }
        ])
